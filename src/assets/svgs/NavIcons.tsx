import { selectLoginWindowActive } from "@store/slices/loginWindowSlice";
import { useSelector } from "react-redux";
import classes from "../../../styles/icons.module.scss";

export interface iconPropTypes {
  onClick?: () => void;
}

export const SearchIcon = ({ onClick }: iconPropTypes) => {
  return (
    <svg
      className={classes.iconNav}
      onClick={onClick}
      width="18"
      height="18"
      viewBox="0 0 18 18"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path d="M11.4918 10.6634C12.4334 9.5357 13 8.08402 13 6.5C13 2.91015 10.0899 0 6.5 0C2.91015 0 0 2.91015 0 6.5C0 10.0898 2.91015 13 6.5 13C7.78297 13 8.97913 12.6283 9.98665 11.9867L15.953 17.953L17.3672 16.5388L11.4918 10.6634ZM11 6.5C11 8.98528 8.98528 11 6.5 11C4.01472 11 2 8.98528 2 6.5C2 4.01472 4.01472 2 6.5 2C8.98528 2 11 4.01472 11 6.5Z" />
    </svg>
  );
};

export const CartIcon = ({ onClick }: iconPropTypes) => {
  return (
    <svg
      className={classes.iconNav}
      onClick={onClick}
      width="20"
      height="20"
      viewBox="0 0 20 20"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path d="M6 16C4.9 16 4.01 16.9 4.01 18C4.01 19.1 4.9 20 6 20C7.1 20 8 19.1 8 18C8 16.9 7.1 16 6 16ZM0 0V2H2L5.6 9.59L4.25 12.04C4.09 12.32 4 12.65 4 13C4 14.1 4.9 15 6 15H18V13H6.42C6.28 13 6.17 12.89 6.17 12.75L6.2 12.63L7.1 11H14.55C15.3 11 15.96 10.59 16.3 9.97L19.88 3.48C19.96 3.34 20 3.17 20 3C20 2.45 19.55 2 19 2H4.21L3.27 0H0ZM16 16C14.9 16 14.01 16.9 14.01 18C14.01 19.1 14.9 20 16 20C17.1 20 18 19.1 18 18C18 16.9 17.1 16 16 16Z" />
    </svg>
  );
};

export const PersonIcon = ({ onClick }: iconPropTypes) => {
  const isLoginWindowActive = useSelector(selectLoginWindowActive);
  return (
    <svg
      className={`${classes["iconNav"]} ${
        isLoginWindowActive && classes.active
      }`}
      onClick={onClick}
      width="16"
      height="16"
      viewBox="0 0 16 16"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path d="M8 8C10.21 8 12 6.21 12 4C12 1.79 10.21 0 8 0C5.79 0 4 1.79 4 4C4 6.21 5.79 8 8 8ZM8 10C5.33 10 0 11.34 0 14V16H16V14C16 11.34 10.67 10 8 10Z" />
    </svg>
  );
};
